...
export default function FormEditorPage({
  ...
  // undo/redo of fields only
  const historyRef = useRef<FormField[][]>([]);
  const futureRef = useRef<FormField[][]>([]);

  useEffect(() => {
    const found = formStore.get(id);
    setDoc(found ?? null);
    if (found) historyRef.current = [found.fields];
  }, [id]);
  ...
  const undo = () => {
    ...
    setDoc((d) => (d ? { ...d, fields: prev } : d));
  };
  const redo = () => {
    const next = futureRef.current.pop();
    if (!next) return;
    historyRef.current.push(next);
    setDoc((d) => (d ? { ...d, fields: next } : d));
  };

  const save = useCallback(
    (overrides?: Partial<FormDoc>) => {
      if (!doc) return;
      const next = { ...doc, ...overrides };
      formStore.upsert(next);
      setSavedAt(Date.now());
    },
    [doc],
  );
  ...
  // Drag-to-move and resize for an existing field
  ...
  useEffect(() => {
    function onMove(e: MouseEvent) {
      ...
      // Don't push to history on every mouse move — set transient state.
      setDoc((d) => (d ? { ...d, fields } : d));
    }
    function onUp() {
      if (!dragRef.current) return;
      dragRef.current = null;
      // Snapshot the latest state into history.
      setDoc((d) => {
        if (d) {
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if ((e.key === "Backspace" || e.key === "Delete") && selectedId && doc) {
        e.preventDefault();
      ...
      <AppShell pageTitle="Form editor">
        <div className="card p-10 text-center">
          <p className="text-ink/70">
            We couldn't find that form. Forms are stored in this browser only.
          </p>
