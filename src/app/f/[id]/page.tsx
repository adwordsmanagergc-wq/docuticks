...
import { fillPdf, downloadDataUrl } from "@/lib/fillPdf";
...
export default function SignerPage({
  ...
  const [done, setDone] = useState<{ filename: string; dataUrl: string } | null>(null);
  ...
  if (doc === null) {
    return (
      <CenteredMessage>
        This form link is no longer available.
      </CenteredMessage>
    );
  }
      ...
      <div className="min-h-screen bg-paper">
        ...
        <div className="container-x flex min-h-[70vh] flex-col items-center justify-center text-center">
          ...
          <p className="mt-2 max-w-md text-ink/65">
            A copy of your completed form has been emailed to the form owner.
            You can also download it for your records.
          </p>
          <p className="mt-2 font-mono text-xs text-ink/55">{done.filename}</p>
          <button
            onClick={() => downloadDataUrl(done.dataUrl, done.filename)}
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const signerName = signerNameField ? values[signerNameField.id] || "Signer" : "Signer";
      const signerEmail = signerEmailField ? values[signerEmailField.id] || "" : "";

      const filename = formatFilename(doc.filenamePattern, {
        formName: doc.name,
        signerName,
      });
      const dataUrl = await fillPdf(doc, values, signatures);
        ...
        <aside className="space-y-4">
          <div className="card p-5">
            ...
            <p className="mt-1 text-xs text-ink/55">
              Complete each field in the form. Required fields are marked
              with *. The form will be auto-named with your name and today's
              date.
            </p>
          </div>
          ...
          {fieldsOnPage
            .filter(
              (f) => f.type === "signature" || f.type === "initials" || activeFieldId === f.id,
            )
            .map((f) => (
              <FieldEditor
                key={f.id}
